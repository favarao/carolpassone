<header>
    <div class="header-mobile">
            <button type="button" class="btn btn-link btn-sm" id="sidebarToggle"><i
                    class="fa-solid fa-bars"></i></button>
            <div class="logo"><a href="/"><img src="images/logo.png" alt="logo"></a></div>
    </div>
    <div class="shade">
        <div class="header-container">
            <div class="menu-nav container">
                <div class="logo"><a href="/"><img src="images/logo.png" alt="logo"></a></div>
                <div class="black-screen" style="display:none"></div>
                <div class="nav">
                    <ul class="nav-links">
                        <li class="nav-item logo-mobile"><a href="/"><img src="images/logo.png" alt="logo"></a></li>
                        <li class="nav-item"><a href="/">Home</a></li>
                        <?php /*
                        <li class="nav-item"><a href="quem-somos.php">Sobre Nós</a>
                            <ul class="menu-group">
                                <li class="sub-item"><a href="quem-somos.php">Sobre</a></li>
                                <li class="sub-item"><a href="nossos-valores.php">Nossos Valores</a></li>
                            </ul>
                        </li>*/?>
                        <li class="nav-item"><a href="e-social.php">Tratamentos</a></li>
                        <li class="nav-item"><a href="seguranca-do-trabalho.php">Quem sou eu</a></li>
                        <li class="nav-item"><a href="medicina-do-trabalho.php">Contatos</a></li>
                    </ul>
                </div>
            </div>
            
            <div class="header-info">
			<div class="header-info-left">
				<div class="header-apresentacao">
					<div class="content">
						<h2 class="page-title">Muito prazer!</h2>
						<h1>Dra. Caroline Passone,</h1>
						<h1>Endocrinopediatra</h1>
						<p>CRM-SP 135.952</p>
					</div>
				</div>
				<div class="header-especialidades">
					<div class="content">
						<ul>
							<li>Especialista em diabetes mellitus tipo 1 e Síndrome de Prader Willi</li>
							<li>Mestre e Doutora em Pediatria e Endocrinologia pela Universidade de São Paulo</li>
							<li>Pós-doutorado em Endocrinologia Pediátrica pela Universidade de Paris- Sorbonne- Hôpital Necker</li>
							<li>Fellowship em Diabetes e Síndrome de Prader Willi na University of Florida -Diabetes Center of Excellence</li>
							<li>Médica do corpo clínico do Hospital Sírio Libanês, Hospital Israelita Albert Einstein e do Instituto da Criança- HCFMUSP ( Endocrinologia Pediátrica e Diabetologia)</li>
						</ul>
					</div>
				</div>
				<div class="header-contato">
					<div class="content">
						<a href="#">Quero agenda minha consulta</a>
					</div>
				</div>
			</div>
			<div class="header-info-right">
				<img src="images/imagem-caroline.png" alt="Dra. Caroline Passone - Endocrinopediatra" title="Dra. Caroline Passone - Endocrinopediatra">
			</div>
		</div>
        </div>
    </div>

</header>